...
def check_disk_space(url="https://yolov8.com/assets/coco8.zip", path=Path.cwd(), sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://yolov8.com/assets/coco8.zip'.
...
def safe_download(
    ...
    if "://" not in str(url) and Path(url).is_file():  # URL exists ('://' check required in Windows Python<3.10)
        ...
        uri = (url if gdrive else clean_url(url)).replace(  # cleaned and aliased url
            ...
            "https://yolov8.com/assets/",  # assets alias
        )
...
def download(url, dir=Path.cwd(), unzip=True, delete=False, curl=False, threads=1, retry=3, exist_ok=False):
    ...
    Example:
        ```python
        download('https://yolov8.com/assets/example.zip', dir='path/to/dir', unzip=True)
        ```
    """
    dir = Path(dir)
